...
static char	*expand_telda(char *command, int *i, t_container *content)
{
	...
	(ft_strncmp(command, "export ", 7) == 0 && command[(*i) - 1] == '='))))
	{
		(*i)++;
...
static char	*check_expanding_variables(t_container *content, char *command, \
int *i, char qoute)
{
	char	*expanded_part;
...
int	check_expanding_telda(t_container *content, char *command, t_vars *vars, \
char qoute)
{
	char	*expanded_part;

	expanded_part = NULL;
	if (qoute != '"' && qoute != '\'' && command[vars->i] == '~' && \
		!vars->here)
	{
...
int	finding_and_expanding(t_container *content, char *command, t_vars *vars, \
char qoute)
{
	...
	qoute && (ft_isalnum(command[vars->i + 1]) || command[vars->i + 1] == '\''\
	|| command[vars->i + 1] == '"' || command[vars->i + 1] == '?' || \
	command[vars->i + 1] == '_'))
	{
		if ((qoute == '"' && command[vars->i + 1] == '\'') || (vars->here \
		&& command[vars->i + 1] != '\'' && command[vars->i + 1] != '"'))
			;
		else
		{
...
char	*check_env_var(t_container *content, char *command)
{
	...
	while (command[vars.i])
	{
		...
		if (finding_and_expanding(content, command, &vars, qoute))
			continue ;
		content->new_command = ft_strchr_join(content->new_command, \
		command[vars.i], content);
		vars.i++;
	}
