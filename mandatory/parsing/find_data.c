...
/*   Updated: 2025/03/22 15:18:36 by kben-tou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_dir_files(char **dir_files, t_token *token, t_container *content)
{
	...
	{
		*dir_files = ft_strjoin(*dir_files, token->value, &content->g_collector, content);
		if (token->next != NULL)
			*dir_files = ft_strjoin(*dir_files, " ", &content->g_collector, content);
		*dir_files = ft_strjoin(*dir_files, token->next->value, &content->g_collector, content);
...
void	get_command(char **only_command, t_token *token, t_container *content)
{
	...
	{
		*only_command = ft_strjoin(*only_command, token->value, &content->g_collector, content);
		if (token && token->next && token->next->token_type != T_PIPE)
			*only_command = ft_strjoin(*only_command, " ", &content->g_collector, content);
	}
}
