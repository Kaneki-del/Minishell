...
/*   Updated: 2025/03/22 03:18:59 by kben-tou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

int	redirection_pipe_check(t_token *iter, t_type_token CASE, t_container \
*content)
{
	int	hold;
	...
	if (iter->token_type == CASE && iter->next->token_type == T_REDIRECTE_OUT)
		return (ft_error("mshell: syntax error near unexpected token", \
		iter->next->value, 2), 0);
	if (iter->token_type == CASE && \
	iter->next->token_type == T_REDIRECTE_HEREDOC)
		return (ft_error("mshell: syntax error near unexpected token", \
		iter->next->value, 2), 0);
	if (iter->token_type == CASE && \
	iter->next->token_type == T_REDIRECTE_APPEND)
		return (ft_error("mshell: syntax error near unexpected token", \
		iter->next->value, 2), 0);
...
int	check_is_last_redirection(t_token *iter, t_container *content)
{
	...
	iter->token_type == T_REDIRECTE_IN \
	|| iter->token_type == T_REDIRECTE_OUT \
	|| iter->token_type == T_REDIRECTE_APPEND \
	|| iter->token_type == T_REDIRECTE_HEREDOC))
	{
		content->status = 258;
		return (ft_error("mshell: syntax error near unexpected token"\
		, "newline", 2), 0);
