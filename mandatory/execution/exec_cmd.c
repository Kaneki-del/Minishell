...
/*   Updated: 2025/03/21 01:36:19 by sait-nac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	executing(t_data *current, t_container *content)
{
	...
	if (!current->cmds || !current->cmds[0])
		exit(0);
	if (content->flag == 5 && (ft_strchr(content->line, '\'')
			|| ft_strchr(content->line, '"')))
	...
	{
		ft_error_exec_two("mshell: ", current->cmds[0], ": command not found",
			2);
