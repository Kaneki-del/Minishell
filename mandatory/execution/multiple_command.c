...
void execute_first(t_data *list, int *p_fd, char **env, t_gc **g_collector) {
  pid_t pid = fork();
  if (pid < 0)
    exit(1);
  if (pid == 0) {
...
int execute_last(t_data *list, int *p_fd, char **env,  t_gc **g_collector) {
  pid_t pid = fork();
  if (pid < 0)
    exit(1);
  if (pid == 0) {
...
static void execut(t_data *list, int *p_fd, char **env, int in ,  t_gc **g_collector) {
  pid_t pid = fork();
  if (pid < 0)
    exit(1);
  if (pid == 0) {
...
static int handle_pipes(t_data **list, char **env,  t_gc **g_collector) {
  int p_fd[2];
  t_data *current = *list;
  ...
  execute_first(current, p_fd, env, g_collector);
  /* if (current->in_fd != 0) */
  /*   close(current->in_fd); */
  /* if (current->out_fd != 0) */
  /*   close(current->in_fd); */
  ...
  while (current != NULL && current->next != NULL) {
    ...
    get_fds(current);
    execut(current, p_fd, env, t, g_collector);
    close(t); // Close previous read end in parent
    current = current->next;
  }

  get_fds(current);
  return execute_last(current, p_fd, env, g_collector);
}

int run_multiple(t_data **list, char **env,  t_gc **g_collector) {
  int status = 0;
  int exit_code = 0;
  int id_last_command = handle_pipes(list, env, g_collector);
